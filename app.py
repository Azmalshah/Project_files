...
# Main heading in centre of the page with a title "Unstop Job Market Trends Analytics"
st.title("Unstop Job Market Trends Analytics") 
...
st.markdown("""
Our project, Unstop Job Market Trends Analytics, focuses on analyzing data from the Unstop platform to uncover valuable insights about internships, jobs, and hackathons. By leveraging the Unstop API, we collect data on key parameters such as eligibility, application deadlines, engagement metrics, and regional preferences. Using advanced preprocessing techniques, we clean and organize this data, ensuring its accuracy and reliability. The final insights are presented through an interactive Streamlit application, featuring dynamic visualizations that help users explore market trends, participation patterns, and hiring demands.
""")

# Data Collection Section
st.markdown("## Data Collection")
st.markdown("""
- **Sources of Data:** We collect data from the Unstop platform, focusing on three main categories: hackathons, jobs, and internships.
- **Methods of Data Scraping:** Using the Unstop API, we fetch data on various parameters such as eligibility criteria, application deadlines, engagement metrics (like impressions and applications), and regional preferences.
- **Data Preprocessing Steps:** 
  - **Cleaning:** Removing duplicates, handling missing values, and correcting data types.
  - **Organizing:** Structuring the data into a consistent format for analysis.
  - **Validation:** Ensuring the accuracy and reliability of the data through validation checks.
""")
...
- **Key Metrics Analyzed:**
  - **Eligibility:** Criteria required for participation in internships, jobs, and hackathons.
  - **Application Deadlines:** Timeframes within which applications need to be submitted.
  - **Engagement Metrics:** Number of impressions and applications received.
  - **Regional Preferences:** Geographic distribution of opportunities.
- **Visualization Techniques Used:**
  - **Bar Charts:** To compare the number of opportunities across different categories.
  - **Heatmaps:** To show correlations between different numeric variables.
  - **Word Clouds:** To visualize the most common eligibility criteria.
  - **Scatter Plots:** To analyze the relationship between impressions and applications.
""")

# Findings Section
st.markdown("## Findings")
st.markdown("""
- **Trends in Hackathon Participation:** Analysis of participation patterns in hackathons, including popular categories and regions.
- **Popular Job Roles and Companies:** Identification of the most sought-after job roles and the companies posting the most opportunities.
- **Internship Opportunities and Eligibility Criteria:** Insights into the types of internships available and the common eligibility requirements.
""")

# Conclusion Section
st.markdown("## Conclusion")
st.markdown("""
- **Summary of Insights:** A recap of the key findings from the analysis.
- **Implications for Students and Professionals:** How the insights can help students and professionals make informed decisions about their career paths.
- **Future Work and Improvements:** Potential areas for further research and ways to enhance the analysis.
""")
...
# Try multiple path options for local and cloud deployment
data_paths = [
    os.path.join(current_dir, 'Preprocessed_files', 'cleaned_hackathons.csv'),
    os.path.join(current_dir, '..', 'Preprocessed_files', 'cleaned_hackathons.csv'),
...
# Convert 'Application Deadline' to datetime and then to a numerical feature
hackathon_data['Application Deadline'] = pd.to_datetime(hackathon_data['Application Deadline'], errors='coerce')
hackathon_data['Application Deadline Timestamp'] = hackathon_data['Application Deadline'].apply(lambda x: x.timestamp() if pd.notnull(x) else None)
...
# Convert 'Application Deadline' to datetime and then to a numerical feature
data_cleaned['Application Deadline'] = pd.to_datetime(data_cleaned['Application Deadline'], errors='coerce')
data_cleaned['Application Deadline Timestamp'] = data_cleaned['Application Deadline'].apply(lambda x: x.timestamp() if pd.notnull(x) else None)

# Visualization 5: Heatmap for numerical columns
st.subheader("5. Correlation Heatmap")
numerical_cols = data_cleaned[['Impressions', 'Applied', 'Application Deadline Timestamp']]
fig, ax5 = plt.subplots(figsize=(8, 6))
sns.heatmap(numerical_cols.corr(), annot=True, cmap="coolwarm", fmt=".2f", ax=ax5)
ax5.set_title('Correlation Heatmap', fontsize=16)
plt.tight_layout()
st.pyplot(fig)
